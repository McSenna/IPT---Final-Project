...
### 5. Optional: Use This Backend with Cursor’s Local AI
---------------------------------------------------------------------------------

If you use **Cursor** with a local model (e.g. pointing to this backend), you can give Cursor a short “system prompt” so it behaves like a ChatGPT-style web chat UI backed by your local server:

```text
You are my local AI assistant. Act like ChatGPT in a web UI that talks only to my local backend at http://localhost:3000/api/chat. 
Stream responses token by token, keep full conversation context, and behave like a standard ChatGPT-style assistant, but assume everything runs 100% locally with no cloud APIs.
```

Paste this into Cursor as the **system / initial instruction** for your local model profile, then simply chat as usual.
